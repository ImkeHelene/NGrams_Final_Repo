...
df = pd.DataFrame.from_csv("shake_genres7.csv")
...
#    cnf_matrix += confusion_matrix(test_labels, predict_labels)
    ...
    for x in range(0, 3):
        ...
        train_labels2=[]
        test_labels2=[] 
        for j in range(0, len(train_speeches)):
            if train_labels[j]==genres[x]:
                train_speeches2.append(train_speeches[j])
                train_labels2.append(train_plays[j])
                
        for i in range(0, len(predict_labels)):
            if predict_labels[i] == genres[x]:
                test_speeches2.append(test_speeches[i])
                test_labels2.append(test_plays[i])
                
        vectorizer.fit(train_speeches2)
        # then we vectorize those speeches
        train_features2 = vectorizer.transform(train_speeches2)       
        test_features2 = vectorizer.transform(test_speeches2)
            
        classifier.fit(train_features2, train_labels2)
        #predict_labels = classifier.predict(test_features)
        NBC_score+= classifier.score(test_features2, test_labels2)
