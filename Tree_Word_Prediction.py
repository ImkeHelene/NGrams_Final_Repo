...
filename = os.getcwd() + '/googlebooks-eng-gb-all-2gram-20120701-wo_Cleaned'

### Establish the tree

# The root node will be the beginning of sentence dummy
root_node = bigram_tree_node(None, '.', 0) 
...
with open(filename, 'rb') as csvfile:
    ...
    for row in reader:
        ...
        if row[0] not in first_list:
            #print('adding Node ' + row[0])
            first_list.append(row[0])
            node_list.append(bigram_tree_node(first_list.index(parent), row[0], index))
            node_list[first_list.index(parent)].add_child(index)  
            if row[2] in first_list:
                node_list[first_list.index(row[0])].add_child(first_list.index(row[2]))
                parent = row[0]
            elif row[2] == '':
                node_list[first_list.index(row[0])].add_child(first_list.index('.'))
                parent = first_list[0]
            else:
                first_list.append(row[2])
                node_list.append( bigram_tree_node( first_list.index(row[0]), row[2], len(first_list) ) )
                node_list[first_list.index(row[0])].add_child(first_list.index(row[2]))
                parent = row[0]
            index +=1
            ...
            # probability count.
            if row[1] in first_list:
                local_child_index = node_list[first_list.index(row[0])].get_child(row[2])#
                if local_child_index  == None:
                    node_list[first_list.index(row[0])].add_child(first_list.index(row[2]))
                else: 
                    node_list[first_list.index(row[0])].probabilities[local_child_index] += 1
            else:
                first_list.append(row[2])
                node_list.append( bigram_tree_node(first_list.index(row[0]), row[2], len(first_list) ) )
...
writefile_name = os.getcwd() + '/tree_file.csv'

children = []
probabilities = []

with open(writefile_name, 'wb') as writefile:
    writer = csv.writer(writefile, delimiter='\t')
    
    writer.writerow(first_list)
    for i in range(len(first_list)):
        children.append(node_list[i].children)  
        probabilities.append(node_list[i].probabilities)
...
avg_num_speakers_per_play = 2
var_range_num_speakers = [0]

avg_num_scenes_per_play = 1
var_range_num_scenes_per_play = [0]

avg_num_words_per_speaker = 50
var_range_num_words_per_speaker =range(-35,35)

avg_num_sentences_per_speech = 1
var_range_num_sentences_per_speech = range(0, 2)

avg_num_speeches_per_speaker = 5
var_range_num_speeches_per_speaker = [0]

avg_num_speakers_per_scene = 2
var_range_num_speakers_per_scene = [0]

avg_num_speeches_per_scene = 5
var_range_num_speeches_per_scene = [0]
