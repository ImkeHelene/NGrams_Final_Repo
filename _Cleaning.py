...
class Clean_NGrams:
    ...
    def rough(self, indx, row, gram): 
        ...
        gram = re.sub("[^\w]", " ",  ).split()
        if ( np.any( row[0:len(gram)] in self.stpwds ) 
            or np.any( float(row[len(gram)+1]) > self.threshold ) 
            or row[0] not in self.dictionary  or row[1] not in self.dicitionary ):
            self.drop = 1
            self.clean_count += 1
        else:
            self.drop = 0
...
file_list = ['/googlebooks-eng-gb-all-2gram-20120701-sh', '/googlebooks-eng-gb-all-2gram-20120701-lo',
	     ...
	     '/googlebooks-eng-gb-all-2gram-20120701-th']
       ...
       # max_count = float(max_count)
        ...
        with open(fname + '_Cleaned' ,'wb') as writefile:
            ...
            for row in reader:
                ...
                Cleaner.rough(indx, row, [row[0], row[2]])
                if Cleaner.drop == 1:
                    continue
                else:
                    Cleaner.types(indx, [row[0], row[2]])                       
